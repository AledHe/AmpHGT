...
from model.utils import compute_accuracy
...
class Masking_Trainer(object):
    ...
    def __init__(self, cfg, global_rank, world_size, model_list, dataloaders,
        ...
        self.root_level_dir = os.path.dirname(os.path.abspath(__file__))
    ...
    def train_epoch(self):
        ...
        for step, (batch, label) in enumerate(self.dataloaders['train']):
            ...
            if self.cfg.train.mask_fragment:
            ...
            if (self.global_train_step +
                ...
                info(
                    "train | epoch: {:d}  step: {:d} | loss: {:.4f} | acc_atom: {:.4f} | acc_pharm: {:.4f} | acc_edge: {:.4f}".format(
            ...
            if (self.global_train_step + 1) % 1500 == 0:
    ...
    def eval_epoch(self, split):
        ...
        if split == "valid":
            ...
            if metrics["{}_{}".format(
                ...
                model_save_path = os.path.join(self.output_dir, self.best_model_path, 'model')
                ...
                if self.global_rank == 0 and self.cfg.logger.log:
                    ...
                    print(f"Root level dir: {self.root_level_dir}")
                    mlflow.pytorch.save_model(
                        pytorch_model=(self.model.module if is_parallel(
                            self.model) else self.model),
                        path=os.path.join(model_save_path, 'model'),
                        code_paths=[
                    ...
                    mlflow.pytorch.save_model(
                        ...
                        path=os.path.join(model_save_path, 'linear_pred_atoms'),
                        code_paths=[
                            os.path.join(self.root_level_dir, "model"),
                            os.path.join(self.root_level_dir, "utils")
                        ])
                    mlflow.pytorch.save_model(
                        ...
                        path=os.path.join(model_save_path, 'linear_pred_pharms'),
                        code_paths=[
                            os.path.join(self.root_level_dir, "model"),
                            os.path.join(self.root_level_dir, "utils")
                        ])
                    ...
                    mlflow.pytorch.save_model(
                        ...
                        path=os.path.join(model_save_path, 'linear_pred_bonds'),
                        code_paths=[
                            os.path.join(self.root_level_dir, "model"),
                            os.path.join(self.root_level_dir, "utils")
                        ])
    ...
    def evaluate(self, split):
        ...
        step = 0
        with torch.no_grad():
            for step, (batch, label) in enumerate(
                    tqdm(self.dataloaders[split],
                         desc="Eval_{}".format(split), unit="batch")):
                batch = batch.to(self.device)
